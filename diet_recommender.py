...
def predict():
    ...
    if goal == 'Weight Loss':
        ...
        return calculate_weight_gain(age, veg_non_veg, weight, height)
    elif goal == 'Healthy':
        return calculate_healthy(age, veg_non_veg, weight, height)
    else:
        return 'Invalid goal'

def calculate_weight_loss(age, veg_non_veg, weight, height):
    ...
    try:
        ...
        response = {
            'message': 'Weight Loss recommendation',
            'food_items': [],
            'diet_type': ''
        }

        for ii in range(len(y_pred)):
            if y_pred[ii] == 2:  # weight loss
                food_item = Food_itemsdata[ii]
                print(f"Food Item: {food_item}")  # Debugging statement
                if int(veg) == 0:
                    datanv = ['Chicken Burger']
                    if food_item not in datanv:
                        response['food_items'].append(food_item)
                else:
                    response['food_items'].append(food_item)

        if int(veg) == 0:
            response['diet_type'] = 'Veg'
...
def calculate_weight_gain(age, veg_non_veg, weight, height):
    ...
    try:
        ...
        response = {
            'message': 'Weight Gain recommendation',
            'food_items': [],
            'diet_type': ''
        }

        for ii in range(len(y_pred)):
            if y_pred[ii] == 2:  # weight gain
                food_item = Food_itemsdata[ii]
                print(f"Food Item: {food_item}")  # Debugging statement
                if int(veg) == 0:
                    datanv = ['Chicken Burger']
...
def calculate_healthy(age, veg_non_veg, weight, height):
    ...
    try:
        ...
        response = {
            'message': 'Healthy recommendation',
            'food_items': [],
            'diet_type': ''
        }
